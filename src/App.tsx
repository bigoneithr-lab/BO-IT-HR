...
export default function App() {
  type ViewType = 'dashboard' | 'employees' | 'access-requests' | 'profile' | 'time-off' | 'departments' | 'ai-assistant' | 'recruitment' | 'settings' | 'payroll' | 'performance' | 'documents' | 'attendance' | 'devices' | 'loans' | 'notices';

  const [currentView, setCurrentView] = useState<ViewType>(() => {
    try {
      const hash = window.location.hash.replace('#', '');
      const validViews = ['dashboard', 'employees', 'access-requests', 'profile', 'time-off', 'departments', 'ai-assistant', 'recruitment', 'settings', 'payroll', 'performance', 'documents', 'attendance', 'devices', 'loans', 'notices'];
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If sign-in fails for the specific admin email, try to create the account
      if (loginEmail === 'bigoneit9326@gmail.com' && (error.code === 'auth/user-not-found' || error.code === 'auth/invalid-credential')) {
        try {
          await createUserWithEmailAndPassword(auth, loginEmail, loginPassword);
          return;
        } catch (createError: any) {
          if (createError.code === 'auth/email-already-in-use') {
            setAuthError('Invalid password for this admin account. Please check your credentials.');
            return;
          }
          // Fall through to general error handling if creation fails for other reasons
        }
      }
    ...
    <div className="flex h-screen bg-[#F0F2F5] overflow-hidden font-sans text-[#333]">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          ...
          {currentView === 'loans' && userRole === 'admin' && <LoanManagement employees={employees} isAdmin={userRole === 'admin'} />}
          ...
          {currentView === 'notices' && (
            <NoticeBoard 
              ...
              currentUserName={user?.displayName || employees.find(e => e.email === user?.email)?.firstName || 'Admin'}
            />
          )}
